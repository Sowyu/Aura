...
struct FilterListCatalogService {
    ...
    /// uBlock Origin's own default selection, plus the AdGuard lists Aura already shipped.
    /// WebKit cannot run uBO itself, so matching its list set is how Aura gets the same coverage.
